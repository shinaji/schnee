...
def test_pcsc_backend_writes_type2_ndef_url() -> None:
    """PC/SC backend writes an NDEF URL TLV to NTAG21x pages."""
    reader = FakeReader()
    backend = PcscBackend(reader=reader)
    select_apdu = (
        0x00,
    ...
    reader.connection.responses = {
        (0xFF, 0xB0, 0x00, 0x03, 0x04): [0xE1, 0x10, 0x12, 0x00],
    }

    backend.write_ndef_url("https://example.com")

    assert reader.connection.commands[-7:] == [
        [0xFF, 0xD6, 0x00, 0x04, 0x04, 0x03, 0x18, 0xD1, 0x01],
        [0xFF, 0xD6, 0x00, 0x05, 0x04, 0x14, 0x55, 0x00, 0x68],
        [0xFF, 0xD6, 0x00, 0x06, 0x04, 0x74, 0x74, 0x70, 0x73],
        [0xFF, 0xD6, 0x00, 0x07, 0x04, 0x3A, 0x2F, 0x2F, 0x65],
        [0xFF, 0xD6, 0x00, 0x08, 0x04, 0x78, 0x61, 0x6D, 0x70],
        [0xFF, 0xD6, 0x00, 0x09, 0x04, 0x6C, 0x65, 0x2E, 0x63],
        [0xFF, 0xD6, 0x00, 0x0A, 0x04, 0x6F, 0x6D, 0xFE, 0x00],
    ]
